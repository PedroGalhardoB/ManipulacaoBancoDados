...
Inserindo os dados e criando uma cópia do banco.
...
Conectando o R a linguagem SQL.

```{r}
db <- dbConnect(SQLite(),"../dados/discoCopy.db")
```

Vizualizando as tabelas que temos no banco "disco".
...
Lista "instruments" do banco.

```{r}
dbListFields(db,'instruments')
```

Removendo a lista "instruments" do banco.

```{r}
dbExecute(db,"DROP TABLE instruments")
```

Lista das tabelas presentes no banco.
...
Temos que criar novamente a lista de instrumentos, pois apagamos ela
...
É importante encerrar suas conexões com dbDisconnect(). Além disso, podemos remover a cópia que fizemos da database disco.db:
...
Você também pode usar a função copy_to(conn, df) do dplyr! A sintaxe é parecida. Vamos agora destruir a conexão e a tabela:
